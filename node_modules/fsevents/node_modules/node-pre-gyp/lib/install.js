...
function do_build(gyp,argv,callback) {
  gyp.todo.push( { name: 'dist', args: ['rebuild'] } );
...
function install(gyp, argv, callback) {
    ...
    var source_build = gyp.opts['dist-from-source'] || gyp.opts.build_from_source;
    ...
    if (should_do_source_build) {
        log.info('dist','requesting source compile');
        return do_build(gyp,argv,callback);
    } else {
        var fallback_to_build = gyp.opts['fallback-to-dist'] || gyp.opts.fallback_to_build;
        ...
        if (process.env.npm_config_argv) {
            ...
            var match = cooked.indexOf("--fallback-to-dist");
            if (match > -1 && cooked.length > match && cooked[match+1] == "false") {
                should_do_fallback_build = false;
                log.info('install','Build fallback disabled via npm flag: --fallback-to-dist=false');
            }
        }
        var opts;
        try {
            ...
            if (found && !update_binary) {
                testbinary(gyp, argv, function(err) {
                    if (err) {
                        ...
                        console.log('Pass --update-binary to reinstall or --dist-from-source to recompile');
                        return callback();
                    }
                });
            } else {
